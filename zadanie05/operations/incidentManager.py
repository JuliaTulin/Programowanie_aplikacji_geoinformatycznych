from fleet import AmbulanceQueue
        ...
        if self.ambulance_queue and self.incident_queue:
            for incident in self.incident_queue:
                ...
                if closest_ambulance:
                    ...
                    Logger.info(f"Ambulance {closest_ambulance.id}, {closest_ambulance.medical_equipment} assigned to incident: {incident.description}")
                else:
                    Logger.warn(f"No available ambulance for incident: {incident.description}")
        else:
            Logger.warn("No available ambulances or incidents.")
